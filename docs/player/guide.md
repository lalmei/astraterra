...
**And it gives you west**, without ever having been told where north was. The sun sets due west at the equinoxes and swings evenly either side of it through the year, so the middle of a finished band of sunsets is due west exactly, at any latitude — the sunrise band gives due east the same way. The disc needs no compass direction to work: everything it computes is a difference between marks, and the first scratch is the reference for all the rest. Which is why the cardinal directions come out of it as a result rather than going into it as a setting.
