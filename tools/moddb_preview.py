...
* ``--preview`` wraps the description in page chrome that imitates mods.vintagestory.at
...
<head>
<meta charset="utf-8" />
<title>AstraTerra — ModDB description preview</title>
<style>
  body {{
    margin: 0;
    padding: 32px 16px;
...
def description(text: str) -> str:
    """The description with the leading how-to-use comment removed."""
...
def resolve_images(text: str, base: Path) -> str:
    def replace(match: re.Match[str]) -> str:
        ...
        figure = '<figure><img src="%s" alt="%s" />' % (src, caption.replace('"', "&quot;"))
        if caption:
